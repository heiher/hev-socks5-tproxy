...
{
	if (self->task_worker_tcp)
		hev_task_run (self->task_worker_tcp, hev_socks5_worker_tcp_task_entry, self);
	if (self->task_worker_dns)
		hev_task_run (self->task_worker_dns, hev_socks5_worker_dns_task_entry, self);
...
hev_socks5_worker_tcp_task_entry (void *data)
{
	...
	for (;;) {
		...
		session = hev_socks5_session_new_tcp (client_fd, session_close_handler, self);
		if (!session) {
			close (client_fd);
			continue;
		}
...
hev_socks5_worker_dns_task_entry (void *data)
{
	...
	for (;;) {
		...
		session = hev_socks5_session_new_dns (self->fd_dns, session_close_handler, self);
		if (!session)
			continue;
